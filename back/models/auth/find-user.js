...
// GET: fielld, value를 통한 회원 데이터-한명
const findUser = async (key, value) => {
	let sql
	try {
		sql = ` SELECT U.*, S.idx AS sidx, S.provider, S.snsName, S.displayName, S.email AS snsEmail, S.profileURL, S.status AS snsStatus, A.domain, A.apikey
		FROM users AS U 
...
const loginUser = async (userid, passwd) => {
	...
	try {
		...
		if(r.length === 1) {
			...
			return { success: false, user: null,msg: '아이디가 일치하지 않습니다.' }
	}
	catch (err) {
		throw new Error(err)
	}
